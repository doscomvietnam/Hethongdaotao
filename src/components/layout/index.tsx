...
export const Sidebar = ({ currentView, setView, employee, collapsed, onToggleCollapse, courseGroup, onSetCourseGroup, mobileOpen, onCloseMobile }: SidebarProps) => {
  ...
  const allMenuItems = [
    ...
    { id: ViewType.LEADERBOARD, icon: Trophy, label: 'Bảng Xếp Hạng', roles: ['admin', 'manager', 'employee'], show: true },
  ...
  React.useEffect(() => {
    if (isCourseView) setCourseMenuOpen(true);
  }, [isCourseView]);
  ...
  return (
    ...
    >
      <div className={cn(
        'relative flex items-center justify-center border-b border-zinc-900/50 transition-all duration-300',
        ...
        <button
          ...
          className="absolute right-4 top-1/2 -translate-y-1/2 md:hidden w-8 h-8 rounded-lg border border-zinc-800 bg-zinc-900/50 flex items-center justify-center text-zinc-500 hover:text-white transition-all"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <nav className={cn(
        ...
        {menuItems.map((item) => {
          // ── Khóa học đào tạo: collapsible với 2 sub-item ──
          if (item.id === ViewType.COURSE_CATALOG) {
            ...
            return (
              <div key={item.id} className="relative group">
                ...
                <button
                  onClick={() => setCourseMenuOpen(p => !p)}
                  className={cn(
                    'w-full flex items-center rounded-2xl font-black transition-all relative',
                    collapsed ? 'justify-center px-0 py-4 text-xs' : 'gap-4 px-5 py-4 text-xs uppercase tracking-widest',
                    isCourseView
                      ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'
                      : 'text-zinc-500 hover:bg-zinc-800/50 hover:text-zinc-200'
                  )}
                >
                  {isCourseView && <div className="absolute left-0 w-1.5 h-6 bg-emerald-500 rounded-r-full shadow-[0_0_15px_#10B981]" />}
                  <GraduationCap className={cn('w-5 h-5 flex-shrink-0', isCourseView ? 'text-emerald-500' : 'text-zinc-600 group-hover:text-zinc-300')} />
                  {!collapsed && (
                    <>
                      <span className="uppercase tracking-widest">Khóa học đào tạo</span>
                      <ChevronDown className={cn('w-3.5 h-3.5 ml-auto opacity-50 transition-transform duration-200', courseMenuOpen ? 'rotate-180' : '')} />
                    </>
                  )}
                </button>
                {/* Sub-items */}
                {courseMenuOpen && !collapsed && (
                  <div className="mt-1 ml-4 space-y-0.5 border-l border-zinc-800 pl-3">
                    <button
                      onClick={() => { setView(ViewType.COURSE_CATALOG); onSetCourseGroup?.('product'); onCloseMobile?.(); }}
                      className={cn(
                        'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all',
                        isProductActive ? 'bg-emerald-500/10 text-emerald-400' : 'text-zinc-600 hover:bg-zinc-800/50 hover:text-zinc-300'
                      )}
                    >
                      <Box className="w-4 h-4 flex-shrink-0" />
                      <span>Khóa học sản phẩm</span>
                    </button>
                    <button
                      onClick={() => { setView(ViewType.COURSE_CATALOG); onSetCourseGroup?.('general'); onCloseMobile?.(); }}
                      className={cn(
                        'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all',
                        isGeneralActive ? 'bg-emerald-500/10 text-emerald-400' : 'text-zinc-600 hover:bg-zinc-800/50 hover:text-zinc-300'
                      )}
                    >
      ...
      <div className={cn(
        ...
        <div
          onClick={() => { setView(ViewType.PROFILE); onCloseMobile?.(); }}
          className={cn(
    ...
    <header className="h-24 border-b border-zinc-900 bg-[#09090B]/80 backdrop-blur-3xl sticky top-0 z-50 px-4 lg:px-8 xl:px-12 flex items-center justify-between">
      <div className="flex items-center gap-3 lg:gap-8 xl:gap-12">
        ...
        <button
          ...
          className="md:hidden w-10 h-10 rounded-xl border border-zinc-800 bg-zinc-900/50 flex items-center justify-center text-zinc-500 hover:text-emerald-500 hover:border-emerald-500/20 hover:bg-emerald-500/5 transition-all flex-shrink-0"
          aria-label="Mở menu"
        >
  ...
  return (
    <div className="flex min-h-screen bg-[#09090B] text-white">
      ...
      <Sidebar
        ...
        onSetCourseGroup={onSetCourseGroup}
