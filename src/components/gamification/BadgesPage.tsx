    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {badges.map((b) => (
          <Card
            key={b.id}
            className={`p-6 rounded-2xl text-center transition-all ${b.earned ? 'bg-[#0C0C0E] border border-emerald-500/20 hover:border-emerald-400/50' : 'bg-[#0C0C0E] border border-zinc-900 opacity-60'}`}
          >
            ...
            <p className={`text-[10px] font-black uppercase tracking-widest mb-2.5 ${b.earned ? 'text-emerald-400' : 'text-zinc-600'}`}>
              {b.earned ? 'Đã đạt ✓' : 'Chưa mở'}
            </p>
